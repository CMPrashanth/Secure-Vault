...
import apiClient from "@/services/api"; // <-- IMPORTED
...
const Dashboard = () => {
        ...
        try {
            ...
            setRecords(response.data);
             ...
             if (error.response?.status === 401) {
                ...
                const errorMessage = error.response?.data?.detail || "Failed to fetch vault data.";
    ...
    const handleDecryptField = useCallback(async (record: Record, fieldName: string) => {
        ...
        try {
            const response = await apiClient.post('/api/vault/decrypt', {
                category: record.type,
                field_name: fieldName
            });
            setDecryptedData(prev => ({ ...prev, [dataKey]: response.data.plaintext }));
        } catch (error: any) {
            const errorMessage = error.response?.data?.detail || "Decryption failed.";
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            await apiClient.put('/api/vault/field', {
                category: record.type,
                field_name: editingField.field,
            ...
            const errorMessage = error.response?.data?.detail || "Failed to update field.";
    ...
    const handleDelete = async () => {
        ...
        const isCategoryDelete = !fieldToDelete;

        try {
            if (isCategoryDelete) {
                await apiClient.delete(`/api/vault/category/${recordToDelete.type}`);
            } else {
                await apiClient.delete('/api/vault/field', {
                    data: {
                        category: recordToDelete.type,
